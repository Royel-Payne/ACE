    ...
    public static class DefaultPropertyManager
    {
        ...
        public static readonly ReadOnlyDictionary<string, Property<bool>> DefaultBooleanProperties =
            DictOf(
                ...
                ("specialty_attribute_gain_enabled", new Property<bool>(false, "(Shadowgain 187 #1, FALLBACK) If enabled, the combat specialties also train their dat attributes instead of paying skill XP alone. Covers DualWield, Recklessness, SneakAttack, DirtyFighting and Deception - the same gap 172 closed for Summoning, where the attribute hook lived only on the attack path. Measured on LIVE: Apex had 226 DualWield awards and 0 Coordination-primary awards. SHIPS OFF because these hooks fire on every qualifying swing for EVERY melee build, so enabling them is a server-wide attribute increase rather than a dual-wield fix. 187 ships dualwield_offhand_credits_weapon_skill as the actual remedy; turn this on only after that has been measured and the remaining gap judged worth closing. Scale with specialty_attribute_weight")),
        ...
        public static readonly ReadOnlyDictionary<string, Property<double>> DefaultDoubleProperties =
            DictOf(
                ...
                ("specialty_attribute_weight", new Property<double>(0.25, "(Shadowgain 187 #1, FALLBACK) Scale on the attribute award from combat specialties, when specialty_attribute_gain_enabled is on. Exists because these hooks fire on EVERY qualifying swing - far more often than the attack hook they sit beside - so full weight would be a large attribute bump across every melee build. Starts at 0.25 per Chris's standing rule that a rate should start low and be raised, because raising is painless and lowering reads as a nerf. UNTUNED: no measurement supports this number yet, it is a starting point for a TEST pass. 0 disables the award while leaving the enable dial on")),

...
# WHERE THE GENERATOR IS WRITTEN. A dry run MUST NOT touch /opt/ACE/sitedata-gen.sh: cron runs that
# exact path every 5 minutes, and rewriting it truncates the file mid-read, so a concurrent cron tick
# execs a partial script and dies with exit 127. That happened on 2026-08-21 18:25:02 during a dry
# run - self-healing, since the next tick succeeded, but the earlier 127s in Logs/sitedata-fail.log
# are almost certainly the same race from previous edits. A rehearsal that can break production is
# not a rehearsal.
GEN=/opt/ACE/sitedata-gen.sh
[ "$DRY" = "1" ] && GEN=/opt/ACE/sitedata-gen-dry.sh
$SSH "$HOST" "cat > $GEN" <<'REMOTE'
#!/usr/bin/env bash
...
if [ "$DRY" = "1" ]; then
  ...
  $SSH "$HOST" 'chmod +x /opt/ACE/sitedata-gen-dry.sh && SG_DATA_OUT=/tmp/sitedata-dry /opt/ACE/sitedata-gen-dry.sh >/dev/null && echo built'
  ...
  $SSH "$HOST" 'rm -rf /tmp/sitedata-dry /opt/ACE/sitedata-gen-dry.sh'
